...
export default function BlogDetail() {
  ...
  useEffect(() => {
    axios.get(`https://s53-indian-heritage-monument-backend-3.onrender.com/api/monuments/${id}`)
      .then(res => setPost(res.data))
      .catch(err => {
        console.error("Error:", err);
        setPost(null); // prevents crash
      });
  }, [id]);
  ...
  return (
    <div style={{ backgroundColor: '#fcf9f5', minHeight: '100vh', padding: '80px 20px' }}>
      <button onClick={() => navigate('/blog')} style={{ marginBottom: '20px', cursor: 'pointer' }}>
        ← Back
      </button>
